...
function batchPublisher(ldr:Key[] keys) returns future<(readonly & any|error)[]> {
    worker publisherWorker returns readonly & PublisherRow[] {
        ...
        sql:ParameterizedQuery query = sql:queryConcat(`SELECT DISTINCT p.id AS id, p.name AS name, p.email AS email
        FROM publishers AS p INNER JOIN books AS b ON p.id = b.publisher
        WHERE publisher IN (`, sql:arrayFlattenQuery(keys), `)`);
